...
export const CashBook: React.FC<CashBookProps> = ({ vouchers, startDate, endDate, onDateRangeChange }) => {
  // Filter cash-related vouchers (Cash-in-Hand ledger entries)
  const cashVouchers = vouchers.filter(voucher => 
    voucher.ledgerEntries.some(entry => 
      entry.ledgerName.toLowerCase().includes('cash') || 
      entry.ledgerName.toLowerCase().includes('cash-in-hand')
    ) &&
  ...
  const cashTransactions = cashVouchers.flatMap(voucher => 
  ...
  const transactionsWithBalance = cashTransactions
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Cash transactions from {formatDate(startDate)} to {formatDate(endDate)}</CardDescription>
      </CardHeader>
      <CardContent>
        {/* Summary Cards */}
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>Analysis of cash movements during the period</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div className="rounded-lg border border-border bg-blue-50/50 dark:bg-blue-900/20 p-4">
                <div className="text-sm text-blue-600 dark:text-blue-400">Cash Turnover Ratio</div>
                <div className="text-2xl font-bold">
            ...
            <div className="mt-4 p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900">
              <div className="text-sm text-green-700 dark:text-green-300">
                Cash Book is {closingBalance >= 0 ? 'in surplus' : 'in deficit'} by {formatCurrency(Math.abs(closingBalance))}
              </div>
            </div>
          </CardContent>
        </Card>
